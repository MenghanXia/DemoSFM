...
void Triangulation::addNewPoint(vector<int> newPointLabel,vector<Point3d> &newPoints)
{
	...
	vector<char> status;
	cloudRadiusFilter(objectPoints,1.0,10,status);
	int Sum = 0;
	double rms1,rms2;
...
//		status.push_back(1);              //don't using clouds-filter result
		if (status[i] == 1)
		{
...
double Triangulation::meanReprojectError(int start, int end, double gapCoeff)
{
	unsigned i,j,k,n;
	vector<Point2d> graphPoints;
	vector<Point3d> objetPoints;
	int step = 1.0/gapCoeff;
	double AveError = 0;
	for (i = start; i <= end; i ++)
	{
